...
class SiloDashboard extends Component {
    ...
    exportToExcel = async () => {
        ...
        if (!startDate || !endDate || !siloType || !reportType) {
            this.setState({ message: "Please select all fields to export.", messageType: "error" });
            setTimeout(() => this.setState({ message: "" }), 3000);
            return;
        }
    ...
    render() {
        ...
        let totalWeight = 0;
        if (reportType === "Received Report") {
            totalWeight = flatData.reduce((sum, item) => sum + (parseFloat(item.WeightAdded) || 0), 0);
        } else if (reportType === "Movement Report") {
            totalWeight = flatData.reduce((sum, item) => sum + (parseFloat(item.WeightTaken) || 0), 0);
        }
        ...
        return (
            <div className="container">
                ...
                {flatData.length > 0 && (
                    <div>
                        <p><strong>Total Records: {flatData.length}</strong></p>
                        <h3>Silo Data</h3>
                        <table>
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                {flatData.map((record, index) => (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{new Date(record.DateTime).toLocaleString("en-GB", {
                                ...
                                <tr style={{ fontWeight: "bold", backgroundColor: "#f0f0f0" }}>
                                    ...
                                    {(reportType === "Received Report" || reportType === "Movement Report") && (
